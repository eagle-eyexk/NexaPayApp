...
export default function PayPage() {
  ...
  if (link.status !== "active") {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="bg-card border border-border/60 rounded-2xl p-8 max-w-sm w-full text-center space-y-4 shadow-sm">
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="bg-card border border-emerald-500/30 rounded-2xl p-10 max-w-md w-full text-center shadow-sm space-y-5">
          ...
          <div className="bg-muted rounded-xl p-5 text-sm space-y-2">
            <div className="flex justify-between"><span className="text-muted-foreground">Amount</span><span className="font-bold text-emerald-700">{parseFloat(link.amount).toFixed(2)} {link.currency}</span></div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button onClick={() => window.open(result.receiptUrl, "_blank")} className="flex items-center justify-center gap-2 w-full py-2.5 bg-muted border border-border/60 rounded-lg text-sm font-medium hover:border-primary/40 transition-colors">
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-5">
        <div className="text-center">
          ...
          <div className="text-xs text-muted-foreground tracking-widest">NEXA · SECURE PAYMENT</div>
        </div>
        ...
        <div className="bg-card border border-border/60 rounded-2xl p-6 space-y-5 shadow-sm">
          ...
          <form onSubmit={handlePay} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-lg transition-all shadow-sm disabled:opacity-60"
            >
          ...
          <div className="text-xs text-muted-foreground text-center">
            🔒 Secured by Nexa · End-to-end encrypted
          </div>
        </div>
      </div>
    </div>
