    ...
    <div className="space-y-6">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" step="any" min="0" required className="w-full bg-muted border border-border/50 rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1.5">Currency</label>
                <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="w-full bg-muted border border-border/50 rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50">
            ...
            <div>
              ...
              <input type="text" value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="What is this payment for?" required className="w-full bg-muted border border-border/50 rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
            <div>
              ...
              <input type="number" value={expires} onChange={(e) => setExpires(e.target.value)} placeholder="e.g. 24" min="1" className="w-full bg-muted border border-border/50 rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
      ...
      {!links || links.length === 0 ? (
        ...
        <div className="space-y-4">
          {links.map((link) => (
            <div key={link.id} className={`bg-card border rounded-xl p-5 transition-colors ${link.status === "active" ? "border-primary/20 shadow-[0_0_15px_rgba(0,255,255,0.05)]" : "border-border/40"}`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {link.paidAt && (
                    <div className="text-xs text-emerald-400 mt-1">Paid on {new Date(link.paidAt).toLocaleString()}</div>
                  )}
                </div>

                <div className="shrink-0 p-2 bg-white rounded-lg shadow">
              ...
              {link.status === "active" && (
                <div className="mt-4 flex items-center gap-2">
                  <div className="flex-1 font-mono text-xs text-muted-foreground bg-muted rounded-lg px-3 py-2 truncate">{link.url}</div>
                  <button
                    onClick={() => copyUrl(link.url, link.id)}
                    className={`p-2 rounded-lg border transition-all ${copiedId === link.id ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "bg-muted border-border/40 text-muted-foreground hover:border-primary/40 hover:text-primary"}`}
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg border border-border/40 bg-muted text-muted-foreground hover:border-red-500/40 hover:text-red-400 transition-all"
                  >
                    <X className="h-4 w-4" />
                  </button>
                </div>
